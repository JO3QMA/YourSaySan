...
- **経路B（文中 blob）**: 経路Aに当てはまらないとき、Discord 向け置換後に Unicode 空白・改行を除去した1本の `blob` を作り、ルーン長が **12〜`SENRYU_MAX_BLOB_RUNES`（既定 100）** のときだけ解析する。`blob` 全体を Kagome `Tokenize`（Viterbi 1本）し、得られた形態素列の **連続部分列**についてモーラ和が 5→7→5（計17モーラ）かつ品詞境界ルールを満たす **最初の** ものをマッチとする（マッチは表層形の連結）。辞書弧の総当たり DFS は用いない。
...
品詞境界の補足: **句頭の接頭詞**（IPA で「お」「ご」等が単独の接頭詞になる場合）は許可する（敬語で始まる句を拾うため）。経路B は **Viterbi の形態素境界にのみ** 句をまたがせられるため、長いひらがなの無区切り連続が1語にまとまると **検出漏れ** しうる（読点などで区切ると分割されやすい）。一方、旧実装のような辞書別解での **部分文字列誤検知は抑えられる**。読みが取れない形態素（UNKNOWN 等）は表面からモーラを推定するため、漢字1字あたりの数え方が実読みとずれることがある。
